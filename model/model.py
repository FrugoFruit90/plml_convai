...
class Model(nn.Module):
    ...
    def build_model(self):
        self.encoder = encoder.EncoderRNN(len(self.input_lang_index2word), self.emb_size, self.hid_size_enc,
                                  self.cell_type, self.depth, self.dropout).to(self.device)

        if self.args.policy == 'softmax':
            self.policy = policy.SoftmaxPolicy(self.hid_size_pol, self.hid_size_enc, self.db_size, self.bs_size).to(
                self.device)
        else:
        ...
        if self.use_attn:
            if self.attn_type == 'bahdanau':
                self.decoder = decoder.SeqAttnDecoderRNN(self.emb_size, self.hid_size_dec, len(self.output_lang_index2word), self.cell_type, self.dropout, self.max_len).to(self.device)
        else:
    ...
    def forward(self, input_tensor, input_lengths, target_tensor, target_lengths, db_tensor, bs_tensor):
        ...
        # TODO TASK 4 build schema of the pipeline
    ...
    def predict(self, input_tensor, input_lengths, target_tensor, target_lengths, db_tensor, bs_tensor):
        with torch.no_grad():
            ...
            #decoder_hidden = (torch.randn(1, input_tensor.size(0), self.args.hid_size_dec),
            #                  torch.randn(1, input_tensor.size(0), self.args.hid_size_dec))
            # GENERATION
